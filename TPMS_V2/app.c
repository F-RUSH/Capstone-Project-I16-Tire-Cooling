...
void iadc_init(void)
{
    ...
    singleInput.negInput = IADC_portPinToNegInput(gpioPortA, 2);
    ...
    (void)result; // Use result.data here
}

static float temp_in_ADC(void)
{
  //ADC_Start(IADC0, adcStartSingle);

  //while (!(ADC_IntGet(IADC0) & ADC_IF_SINGLE))
  {
    //wait
  }

  uint32_t raw = ADC_DataSingleGet(IADC0);

  float v = raw * 3.3f; // use conversion for ADC -> volt on data sheet

  float temp = v * 2.2f; // use conversion from temp sensor data sheet
